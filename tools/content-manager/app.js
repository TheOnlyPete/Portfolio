...
function blockInspector(b){
  ...
  if(b.type==="button")controls+=field("Button label","label",b.label||"Learn more")+field("Destination URL","url",b.url||"")+field("Shape","shape",b.shape||"square","select","square|rounded|pill")+field("Hover effect","hoverEffect",b.hoverEffect||"default","select","default|fade|fill")+`<label class="field">Hover colour<input type="color" data-key="hoverColor" value="${safeColor(b.hoverColor)}"></label><p class="hint">“Fill” sweeps the colour in from the left. “Fade” changes the whole button colour smoothly.</p>`;
