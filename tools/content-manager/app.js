...
const defaults = {
  ...
  button:{type:"button",label:"Learn more",url:"/contact"}
};
...
function blockHTML(block){
  ...
  if(block.type==="button") return `<div class="story-button-preview"><span>${esc(block.label||"Learn more")}</span><i>→</i></div>`;
  return "";
}
...
function blockInspector(b){
  ...
  if(b.type==="button")controls+=field("Button label","label",b.label||"Learn more")+field("Destination URL","url",b.url||"");
